...
import 'package:asoyy/features/finance/services/receipt_scanner.dart';
...
void main() {
  ...
  testWidgets('the platform answers the recognition channel', (tester) async {
    final file = await _renderReceipt();
    final lines = await ReceiptScanner.recognizeLines(file.path);
    expect(lines, isNotEmpty, reason: 'the recogniser read nothing at all');
  });
  ...
  testWidgets('reads a rendered receipt into a usable scan', (tester) async {
    ...
    final scan = parseReceipt(groupIntoVisualRows(lines));
